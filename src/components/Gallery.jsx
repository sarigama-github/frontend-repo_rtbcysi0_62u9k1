export default function Gallery({ t }) {
  ...
  return (
    <section id="gallery" className="py-16 bg-gradient-to-b from-rose-50 via-amber-50 to-teal-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <span className="inline-block rounded-full bg-white/70 px-3 py-1 text-xs font-semibold text-slate-700 shadow-sm mb-3">
            {t.gallery.subtitle}
          </span>
        ...
        <div className="relative overflow-hidden rounded-2xl shadow mb-6">
          <img src={images[0]} alt="Kids playing happily" className="w-full h-64 md:h-96 object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/30 via-black/0 to-black/0" />
          <div className="absolute bottom-3 left-3 right-3 flex flex-wrap gap-2">
            <span className="inline-flex items-center rounded-full bg-white/80 text-slate-800 text-xs font-bold px-3 py-1">
              Play • Friendship • Smiles
            </span>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
          {images.slice(1).map((src, i) => (
            <div key={i} className="relative overflow-hidden rounded-2xl shadow-sm border border-white/60 bg-white/60">
              <img src={src} alt="Gallery" className="aspect-square w-full object-cover" />
